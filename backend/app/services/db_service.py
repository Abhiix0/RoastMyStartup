...
class DatabaseService:
    ...
    def get_user_roast_count(self, user_id: str) -> int:
        """
        Get the number of roasts generated by a specific user.
        Returns 0 if user_id is None or query fails.
        """
        if not user_id:
            return 0
        try:
            result = self.supabase.table("roasts").select("id", count="exact").eq("user_id", user_id).execute()
            return result.count if result.count is not None else 0
        except Exception as e:
            logger.error(f"Failed to get roast count for user {user_id}: {str(e)}")
            return 0
